...
//	private int m_targetRadius = 15; // TODO брать радиус из кричера?
	 ...
	 * Конструктор.
	 */
	public StateShoot(Player player) {
		super();
		...
		if (!m_exit) {
			if (m_targetLine != null) {
				for (Point p : m_targetLine) {
					StateGameplay.getViewport().drawString("x", p.x, p.y,
							m_targetValid ? Color.yellow : Color.red,
							Color.black, g);
				}
			}
			StateGameplay.getViewport().drawString("X", m_targetPos.x,
					m_targetPos.y, m_targetValid ? Color.yellow : Color.red,
					Color.black, g);
			// DEBUG
			g.setColor(Color.yellow);
			g.drawString("Target: " + m_targetPos.x + ", " + m_targetPos.y,
					100, 15);
		}
	}
	...
	public void onKeyPressed(int key, char c) {
		if (!m_exit) {
			...
			if (Helper.moveMapPointKeyboard(m_targetPos, key, c)) {
				...
				m_targetValid = m_los.existsLineOfSight(m_moveAdapter,
						m_playerPos.x, m_playerPos.y, m_targetPos.x,
						m_targetPos.y, true);
			...
			if (m_targetValid
				...
				AnimationBulletFlight animation = new AnimationBulletFlight(
						StateGameplay.getViewport(), m_targetLine, 20);
